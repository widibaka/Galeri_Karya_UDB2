				...
				<table id="example2" class="table table-bordered table-hover">
					...
					<?php $number = 1; ?>
						<?php foreach ($main_data as $key => $val): ?>
							...
							<?php endforeach ?>
								<td>
									<?php echo form_open( base_url() . 'admin/api/buka_blokir_akun/' ) ?>
										<input type="hidden" name="id_user" value="<?php echo $val['id_user'] ?>">

										<div class="btn-group">
											<button type="submit" href="" class="btn btn-success" title="Hapus" onclick="return confirm('Yakin ingin membuka blokir akun ini?')"><i class="fas fa-user-slash"></i> Buka Blokir</button>
										</div>
									</form>
